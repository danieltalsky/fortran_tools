...
class Fixed2Free:
    ...
    # misc regex's
    re_line_continuation = re.compile(r"^(\s\s\s\s\s)([^\s])([\s]*)")
    re_hollerith = re.compile(r"[\s,]((\d+)H)")
    re_number_spacing = re.compile(r"([^A-Za-z][\d.]+)[\s]([\d.]+[^A-Za-z])")
    re_f77_comment = re.compile(r"^[cC]")
    re_f90_comment = re.compile(r"^!")

    # variable regex
    re_var_end = re.compile(r".*[A-Za-z0-9]+$")
    re_var_begin = re.compile(r"(^\s\s\s\s\s)([^\s])([\s]*)([A-Za-z]+[0-9]?)+")

    # number regex
    re_number_end = re.compile(r".*[\d.eE]+$")
    re_number_begin = re.compile(r"(^\s\s\s\s\s)([^\s])([\s]*)[\d.eE]+")

    # exponent regex
    re_exponent_old = re.compile(r"([\d.]E)[\s]([\d]+)")
    ...
    def fix_line_continuation(line, prev_line):
        ...
        if continuation_type == "hollerith":
            line = Fixed2Free.re_line_continuation.sub(r"\g<1>&\g<3>", line, count=1)
    ...
    def fix_exponents(line):
        return Fixed2Free.re_exponent_old.sub(r"\g<1>+\g<2>", line)

    @staticmethod
    def fix_number_spacing(line):
        return Fixed2Free.re_number_spacing.sub(r"\g<1>\g<2>", line)
    ...
    def find_continuation_type(line, prev_line):
        ...
        # var continuation
        if Fixed2Free.re_var_begin.match(line) and Fixed2Free.re_var_end.match(
            prev_line
        ):
            return "var"

        # number continuation
        if Fixed2Free.re_number_begin.match(line) and Fixed2Free.re_number_end.match(
            prev_line
        ):
            return "number"
    ...
    def from_argv(cls, argv):
        ...
        parser.add_argument(
            "input_filename", metavar="input_file", help="the input filename"
        )
        parser.add_argument(
            "output_filename", metavar="output_file", help="the output filename"
        )
        parser.add_argument(
            "--style",
            dest="style",
